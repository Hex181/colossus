...
const booksAddress = "0xaB914125E2156DdAF5bC41691CF9325F2c9c09E1";
const booksAbi = [
    {
        "inputs": [],
        "name": "getBooks",
...
const getBooks = async (amount) => {
    ...
    try {
        const provider = new ethers.providers.JsonRpcProvider("https://rpc-testnet.5ire.network/");
...
const addFile = async () => {
    ...
    if (file && file.name.slice(-3) == "pdf") {
        ...
        try {
            ...
            const provider = new ethers.providers.JsonRpcProvider(`https://rpc-testnet.5ire.network/`);
            // const provider = new ethers.providers.Web3Provider(`https://rpc-testnet.5ire.network/`);
            var signer = new ethers.Wallet(private_key, provider);
            // console.log(signer)
            const booksContract = new ethers.Contract(booksAddress, booksAbi, signer);
            console.log({ booksContract });
